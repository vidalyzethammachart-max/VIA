...
export function SectionCard({
  ...
  const unansweredCount = section.questions.filter(
    (question) => typeof answers[question.label] !== "number",
  ).length;
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200">
        ...
        <div className="divide-y divide-slate-100 text-xs md:text-sm">
          {section.questions.map((question: Question, idx: number) => {
            const isAnswered = typeof answers[question.label] === "number";
            ...
            return (
              ...
              >
                ...
                <div className="flex border-t border-slate-50 md:contents md:border-t-0">
                  {[1, 2, 3, 4, 5].map((value) => {
                    const selected = answers[question.label] === value;

                    return (
                      <button
                        key={value}
                        type="button"
                        onClick={() => onToggle(question.label, value as LikertValue)}
